...
async function safeReply(int, opts) {
  try { return await int.reply({ ...opts, flags: MessageFlags.Ephemeral }); }
  catch { return await int.reply({ ...opts, ephemeral: true }); }
}
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // Friendly perms heads-up
    const ch = interaction.channel;
    const me = ch?.guild?.members?.me;
    const canSpeak = me && ch.permissionsFor(me)?.has(
      PermissionsBitField.Flags.ViewChannel |
      PermissionsBitField.Flags.SendMessages |
      PermissionsBitField.Flags.EmbedLinks
    );
    if (!canSpeak) {
      await safeFollowUp(interaction, { content: 'Heads up: I may not post here; I’ll try a different channel in this server.' });
    }
    ...
    // notify() lets the lobby tell the host where it posted / any fallback
