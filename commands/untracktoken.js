...
module.exports = {
  ...
  async execute(interaction, { pg }) {
    ...
    if (result.rowCount === 0) {
      await interaction.reply({ content: `❌ No tracked token found for \`${input}\` in this server.`, ephemeral: true });
      return;
    }

    const deleted = result.rows[0];
    let msg = `✅ Untracked \`${deleted.name}\` (${deleted.address}) from this server.\n`;
    ...
    if (remaining.rowCount === 0) {
      msg += `🧼 No tokens are currently tracked in this server.`;
    } else {
      msg += `📡 Still tracking:\n` +
