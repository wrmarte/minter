...
/* ---------- Provider & scoring ---------- */
function toStaticNetwork(key) {
  const meta = CHAIN_META[key] || null;
  if (!meta?.network?.chainId) return null;
  try {
    // ethers v6 expects a Network instance for staticNetwork (NOT boolean)
    return Network.from(meta.network);
  } catch {
    try {
      return new Network(String(meta.network.name || key), Number(meta.network.chainId));
    } catch {
      return null;
    }
  }
