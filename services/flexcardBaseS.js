...
const { fetchMetadataExtras } = require('../utils/fetchMetadataExtras'); // ✅ extra info
...
// 🟩 Base provider (can be swapped per network)
...
async function buildFlexCard(contractAddress, tokenId, collectionName) {
  ...
  // ✅ Metadata extras (rank, minted, network, supply)
  ...
  return await generateFlexCard({
    ...
    owner: ownerDisplay,
    openseaUrl,
    ...extras // ✅ Injected as rank, minted, totalSupply, network
  });
}
