...
const provider = new JsonRpcProvider('https://apechain.drpc.org');
...
async function fetchMetadata(contractAddress, tokenId) {
  try {
    ...
    console.error('❌ Metadata fetch failed (APE):', err);
    return {};
  }
}
...
async function fetchOwner(contractAddress, tokenId) {
  try {
    ...
    console.error('❌ Owner fetch failed (APE):', err);
...
async function buildFlexCard(contractAddress, tokenId, collectionName) {
  ...
  const openseaUrl = `https://opensea.io/assets/ape/${contractAddress}/${tokenId}`;
