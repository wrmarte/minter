...
const ROUTERS = [
  ...
  '0x95ebfcb1c6b345fda69cf56c51e30421e5a35aec'  // Smart deployer on Base
];
    ...
    provider.on('block', async (blockNumber) => {
      ...
      try {
        ...
        for (const log of logs) {
          ...
          if (!ROUTERS.includes(lowerFrom)) continue; // must be from router
          if (to.toLowerCase() === '0x0000000000000000000000000000000000000000') continue; // ignore burns

          const tokenAmount = parseFloat(formatUnits(amount, 18));
          if (!tokenAmount || tokenAmount < 1) continue; // skip dust

          const [usdPrice, marketCap] = await Promise.all([
            getTokenPriceUSD(address),
            getMarketCapUSD(address)
          ]);

          const usdValue = tokenAmount * usdPrice;
          const ethValue = await convertUSDToETH(usdValue);
          ...
          const embed = new EmbedBuilder()
            ...
            .addFields(
              ...
              { name: '💵 Price', value: `$${usdPrice.toFixed(8)}`, inline: true },
              { name: '📊 MCap', value: `$${Math.round(marketCap).toLocaleString()}`, inline: true }
            )
            .setColor(0xff4444)
...
async function getTokenPriceUSD(address) {
  try {
    ...
    console.warn('❌ USD price fetch failed:', err.message);
    return 0;
  }
}
...
async function getMarketCapUSD(address) {
  try {
    ...
    console.warn('❌ Market cap fetch failed:', err.message);
    return 0;
  }
}
