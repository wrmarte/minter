...
async function buildFlexCard(contractAddress, tokenId, collectionName) {
  ...
  let nftImageUrl = metadata?.image || metadata?.image_url || 'https://i.imgur.com/EVQFHhA.png';
  ...
  // 🔍 Find top trait (non-numeric preferred)
  const topTrait = Array.isArray(metadata?.attributes)
    ? metadata.attributes.find(t => typeof t.value === 'string')?.trait_type || 'N/A'
    : 'N/A';
  ...
  // 🧠 Extra metadata: mint date, rarity, network, total supply, mint price, floor price
  ...
  return await generateFlexCard({
    ...
    openseaUrl,
    topTrait,
    ...extras // Injects: minted, rank, score, network, totalSupply, mintPrice, floorPrice
  });
}
